...
namespace {
bool audioMediaFallback(bool& player,bool& browser) {
    ...
    for(UINT i=0;i<count;i++) {
        ...
        for(int j=0;j<sessionCount;j++) {
            ...
            if(QueryFullProcessImageNameW(process,0,path,&length)) {
                std::wstring exe=path; auto separator=exe.find_last_of(L"\\/"); if(separator!=std::wstring::npos) exe=exe.substr(separator+1);
                for(auto& ch:exe) ch=static_cast<wchar_t>(towlower(ch));
                if(exe==L"chrome.exe" || exe==L"msedge.exe" || exe==L"firefox.exe" || exe==L"brave.exe" || exe==L"opera.exe" || exe==L"browser.exe" || exe==L"vivaldi.exe") browser=true;
                if(exe==L"vlc.exe" || exe==L"mpv.exe" || exe==L"mpc-hc64.exe" || exe==L"mpc-hc.exe" || exe==L"mpc-be64.exe" || exe==L"potplayermini64.exe" || exe==L"wmplayer.exe" || exe==L"microsoft.media.player.exe") player=true;
            }
            CloseHandle(process);
        }
    }
...
void MediaMonitor::start() {
    ...
    worker=std::thread([this] {
        try {
            ...
            bool initialVideo=false,initialBrowser=false;
            ready=audioMediaFallback(initialVideo,initialBrowser); video=initialVideo; unknown=initialBrowser;
            ...
            while(true) {
                ...
                bool v=false,u=false; bool supported=audioMediaFallback(v,u);
                try {
                    if(manager) for(const auto& session:manager.GetSessions()) {
                        ...
                        if(type && type.Value()==MediaPlaybackType::Video) v=true;
                        else u=true; // Browsers may mark video as music; conservative mode covers all playing media.
                    }
                    supported=supported || static_cast<bool>(manager);
                } catch(...) { manager=nullptr; }
                video=v; unknown=u; ready=supported;
...
Activity currentActivity(const MediaMonitor& media,const Settings& s,HWND own) {
    // Only a session Windows actually calls video outranks a fullscreen app. Games keep an
    // audio session open, so the conservative "something is playing" rule used to label every
    // game as video and silently ignore the separate game delay.
    if(media.playingRealVideo()) return Activity::Video;
    HWND fg=GetForegroundWindow();
    if(!fg || fg==own || fg==GetDesktopWindow() || fg==GetShellWindow() || IsIconic(fg)) return Activity::Desktop;
    wchar_t cls[64]{}; GetClassNameW(fg,cls,64);
    if(!wcscmp(cls,L"WorkerW") || !wcscmp(cls,L"Progman") || !wcscmp(cls,L"Shell_TrayWnd")) return Activity::Desktop;
    DWORD id=0; GetWindowThreadProcessId(fg,&id);
    HANDLE process=OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION,FALSE,id);
    if(process) {
        wchar_t path[32768]{}; DWORD length=32768;
        if(QueryFullProcessImageNameW(process,0,path,&length)) {
            std::wstring exe=path; size_t separator=exe.find_last_of(L"\\/"); if(separator!=std::wstring::npos) exe=exe.substr(separator+1);
            for(auto& ch:exe) ch=static_cast<wchar_t>(towlower(ch));
            // Fallback for players that do not publish a Windows media session.
            if(exe==L"vlc.exe" || exe==L"mpv.exe" || exe==L"mpc-hc64.exe" || exe==L"mpc-hc.exe" || exe==L"mpc-be64.exe" || exe==L"potplayermini64.exe") { CloseHandle(process); return Activity::Video; }
        }
        CloseHandle(process);
    }
    ...
    if(GetWindowRect(fg,&r)&&GetMonitorInfoW(MonitorFromWindow(fg,MONITOR_DEFAULTTONEAREST),&info)) {
        ...
        if(r.left<=m.left+2&&r.top<=m.top+2&&r.right>=m.right-2&&r.bottom>=m.bottom-2) return Activity::Game;
    }
    // Nothing fullscreen: an unknown media session now counts as video, if asked to.
    if(media.playingVideo(s.mediaConservative)) return Activity::Video;
